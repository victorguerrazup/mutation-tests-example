...
class ChildProcessTestRunnerWorker {
    ...
    async dryRun(options) {
        ...
        if (dryRunResult.status === test_runner_1.DryRunStatus.Error) {
            dryRunResult.errorMessage = (0, util_1.errorToString)(dryRunResult.errorMessage);
        }
        return dryRunResult;
    }
    async mutantRun(options) {
        ...
        if (result.status === test_runner_1.MutantRunStatus.Error) {
            result.errorMessage = (0, util_1.errorToString)(result.errorMessage);
        }
        return result;
    }
}
exports.ChildProcessTestRunnerWorker = ChildProcessTestRunnerWorker;
ChildProcessTestRunnerWorker.inject = (0, plugin_1.tokens)(plugin_1.commonTokens.options, plugin_1.commonTokens.injector);
